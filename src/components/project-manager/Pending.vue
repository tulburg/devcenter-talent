		...
		<div class="list" v-else>
			...
			<div class="project-view-pane">
				...
				<ProjectView v-if="selected!=undefined" :data="{
					...
					shared_with: (selected.team_members) ? selected.team_members.map((t) => { return t.first_name+' '+t.last_name; }) : []
				}" :actions="[
			...
			<div class="project-talent-pane" v-if="selected!=undefined">
				...
				<div class="project-talents">
					...
					<div class="right" v-if="!talentsLoading">
						<!-- sorry for this -->
						<div :class="{ disabled: (selected.team_members) ? selected.team_members.find((t)=>{ return t.id==talent.id })!=undefined : false}" class="box talent-profile-card" v-for="talent in talents" v-if="(
...
<script>
	...
	export default {
		...
		data() { 
			return { 
				user: undefined, projects: [], selected: undefined, showModal: false, name: 'Tolu', 
				showDatePickerAlert: false, savingProject: false, showArchiveModal: false, 
				archiveLoading: false, archiveReason: '', stacks: [], platforms: [], 
		...
		methods: {
			...
			setValue(type, value, item) {
				...
				console.log(this.selectedRoles, this.selectedLangauges, this.selectedEmploymentStatus);
			},
			openTalentPane(type) {
				this.fetchTalents();
